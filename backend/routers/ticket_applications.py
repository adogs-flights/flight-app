...
@router.post(
    "/tickets/{ticket_id}/applications",
    response_model=schemas.TicketApplication,
...
def create_application_for_ticket(
    ...
    if ticket.status != "sharing":
        raise HTTPException(
            ...
            detail="This ticket is not available for sharing",
        )

    if ticket.owner_id == current_user.id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="You cannot apply for your own ticket",
        )
    ...
    if existing_application:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="You have already applied for this ticket",
        )
...
@router.get(
    "/tickets/{ticket_id}/applications", response_model=list[schemas.TicketApplication]
)
def list_applications_for_ticket(
    ...
    if not is_owner and not is_admin:
        raise HTTPException(
            ...
            detail="Not authorized to view applications for this ticket",
        )
...
def update_application_status(
    ...
    application = (
        db.query(models.TicketApplication)
        .filter(models.TicketApplication.id == application_id)
        .first()
    )
    if not application:
        raise HTTPException(
    ...
    if new_status == "confirmed" and original_status != "confirmed":
        ...
        body_rejected = (
            f"<h3>아쉽지만 '{ticket.title}' 티켓 나눔 대상으로 "
            "선정되지 않았습니다.</h3>"
            "<p>다음에 더 좋은 기회로 만나 뵙기를 바랍니다.</p>"
        )
...
def delete_application(
    ...
    if application.status != "pending":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Cannot delete an application that is not in pending state",
        )
